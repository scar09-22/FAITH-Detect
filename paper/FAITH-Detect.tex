...
\end{tabular}
\end{table}
With generator-diverse, same-domain training data, the baseline retains a 6-point advantage on held-out generators, so part of the transfer gap is attributable to the invariance constraint itself, not only to domain shift (Figure~\ref{fig:06c_heldout_generator}).
\begin{figure}[H]\centering
...
\end{tabular}
\end{table}
At this scale, masking \emph{auxiliaries} produces the lowest OOD transfer (48\%) and masking \emph{pronouns} the highest (54\%) (Figure~\ref{fig:16_category_ablation}). We treat this ordering as suggestive rather than conclusive given the pilot scale, and release the protocol for replication at full scale.
\begin{figure}[H]\centering
...
\end{tabular}
\end{table}
HC3 is near-ceiling for both variants (ChatGPT-era answers are stylistically blatant), so in-domain parity replicates trivially. The informative result is transfer: here Hard-Mask transfers \emph{better} than the baseline --- the opposite direction from the hotel$\rightarrow$movie result --- indicating the sign of the invariance--transfer effect is corpus-dependent rather than a universal penalty. We report both directions and claim only the trade-off's existence, not its sign.
\subsection{Modern open-weight generators}
We generate fresh same-domain hotel reviews with locally run open-weight LLMs (Llama-3.2-3B, Gemma-2-2B; prompted with real hotel names and cities) and evaluate the trained detectors per generator (Figure~\ref{fig:18_new_generators}). Closed API models (GPT-4o, Claude, Gemini) are out of scope locally and remain future work.
\begin{figure}[H]\centering
