...
\usepackage{graphicx,booktabs,amsmath,amssymb,float,caption,times}
\usepackage[hidelinks]{hyperref}
\graphicspath{{figures/}{results_colab/figures/}}
\title{\textbf{Should a Detector Ignore Function Words? Characterising the Trade-offs of Function-Word-Invariant Detection of AI-Generated Reviews}}
...
\end{tabular}
\end{table}
For context we evaluate four training-free statistical detectors---mean log-likelihood, log-rank \cite{gltr}, predictive entropy, and the analytic Fast-DetectGPT discrepancy \cite{fastdetectgpt}---scored with GPT-2 and thresholded on the training split only.\footnote{Our entropy score is oriented so that \emph{lower} entropy indicates machine text, which is the empirically correct direction on this dataset and the opposite of the convention in some prior work; AUROC is unaffected by orientation up to reflection.} They trail the supervised models in-domain, as expected on a single-domain benchmark, and provide a reference point that requires no labelled data. We report them to contextualise the supervised results rather than as competitors.
Under the function-word attack the training-free detectors shift by +35.9 F1 points on average, confirming that likelihood-based statistics also respond to function-word perturbations; only the hard-masked model is invariant to them by construction.
...
Attributing the hard-masked model's decisions directly exposes the mechanism: in-domain, 50\% of its attribution mass falls on its 50 most-attributed in-domain content words, but on out-of-domain text only 3\% of the attribution touches that vocabulary---the content words the model learned to use are simply absent (accuracy 90\% in-domain vs.\ 55\% OOD on the diagnostic sample; Figure~\ref{fig:15_ood_diagnostic_hardmask}). This supports a data explanation (domain-specific content vocabulary) over a purely structural one, motivating the mixed-generator experiment below.
\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{figures/15_ood_diagnostic_hardmask.png}
...
To separate generator shift from domain shift, we add same-domain reviews from held-in generators (llama-chat, mistral, mpt-chat, cohere; plus half of the human reviews) to the training set and evaluate on a disjoint frame of held-out generators (gpt3, gpt2, mistral-chat, mpt; plus the remaining humans). Held-out-generator evaluation is then same-domain, different-generator. (Pilot scale: distilroberta-base, 5 seeds, 600-review training subsample.)
\begin{table}[H]\centering
...
\end{tabular}
\end{table}
With generator-diverse, same-domain training data, the baseline retains a 4-point advantage on held-out generators, so part of the transfer gap is attributable to the invariance constraint itself, not only to domain shift (Figure~\ref{fig:06c_heldout_generator}).
\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{figures/06c_heldout_generator.png}
...
We retrain the hard-masked detector masking ONE grammatical category at a time (determiners, pronouns, prepositions, conjunctions, auxiliaries, adverbial function words), with the full union mask as reference. (Pilot scale: distilroberta-base, 5 seeds, 600-review training subsample.)
\begin{table}[H]\centering
...
\end{tabular}
\end{table}
At this scale, masking \emph{auxiliaries} produces the lowest OOD transfer (45\%) and masking \emph{prepositions} the highest (50\%) (Figure~\ref{fig:16_category_ablation}). We treat this ordering as suggestive rather than conclusive given the pilot scale, and release the protocol for replication at full scale.
\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{figures/16_category_ablation.png}
...
We compute the Monroe et al.\ ``Fightin' Words'' log-odds ratio with an informative Dirichlet prior for every function word, contrasting AI and human text in-domain, and repeat the statistic on the out-of-domain reviews pool to test the \emph{stability} of each signal.
\begin{table}[H]\centering
...
\end{tabular}
\end{table}
Only 69\% of significant ($|z|>2$) function-word signals keep their sign across domains; the canonical article ``the'' is the strongest AI-leaning signal in-domain yet reverses out-of-domain, while pronouns (humans write ``I''/``we'' in every domain) form the stable family (Figure~\ref{fig:17_fw_words}). This is the word-level statistical basis of the shortcut claim: a large share of the function-word signal an unconstrained detector absorbs is domain-local.
\begin{figure}[H]\centering
\includegraphics[width=0.98\linewidth]{figures/17_fw_words.png}
...
We retrain baseline and Hard-Mask from scratch on HC3 (human vs.\ ChatGPT answers) and evaluate in-corpus, under the function-word attack, and zero-shot transferred to MAiDE-up.
\begin{table}[H]\centering
\caption{Replication on HC3 (mean $\pm$ 95\% CI over seeds; \% units).}
\label{tab:hc3}
\small
\begin{tabular}{lccc}
\toprule
Variant & HC3 in-domain F1 & FW-attack F1 & $\rightarrow$MAiDE F1 \\
\midrule
Baseline & 100.0 $\pm$ 0.0 & 100.0 $\pm$ 0.0 & 49.3 $\pm$ 105.0 \\
Hard-Mask & 100.0 $\pm$ 0.0 & 100.0 $\pm$ 0.0 & 63.6 $\pm$ 37.1 \\
\bottomrule
\end{tabular}
\end{table}
\paragraph{Why HC3 reaches ceiling.} Both variants reach (near-)perfect in-corpus F1, which we examined rather than celebrated. Three factors contribute. (i) HC3's ChatGPT-era answers carry blatant register markers (uniform length, hedging boilerplate, list-like structure) that its own release paper documents as highly separable. (ii) Our splits are grouped by \emph{question}: HC3 contains several answers per question, and a naive row-level split would leak question context across train/test --- we found and removed exactly this hazard (answers to one question never straddle the boundary), and the ceiling persists, so it is a property of the corpus, not leakage. (iii) The classifier is fine-tuned on in-corpus data; ceiling separability under fine-tuning is consistent with prior HC3 results. Consequently the in-corpus and attack rows certify little beyond corpus easiness; the informative result is transfer: Hard-Mask transfers \emph{better} than the baseline here --- the opposite direction from the hotel$\rightarrow$movie result --- indicating the sign of the invariance--transfer effect is corpus-dependent rather than a universal penalty. We claim only the trade-off's existence, not its sign.
\subsection{Modern open-weight generators}
We generate fresh same-domain hotel reviews with locally run open-weight LLMs (Llama-3.2-3B, Gemma-2-2B) and with Claude (Haiku/Sonnet tiers via its CLI), all prompted with real hotel names and cities, and evaluate the trained detectors per generator (Figure~\ref{fig:18_new_generators}). GPT-4o and Gemini require API access we do not have and remain future work.
\begin{figure}[H]\centering
\includegraphics[width=0.9\linewidth]{figures/18_new_generators.png}
...
For context we evaluate two public pre-trained detectors zero-shot (no fine-tuning; they were trained on other distributions, so this measures transfer \emph{into} our setting, not their best case).
\begin{table}[H]\centering
...
\end{tabular}
\end{table}
Both transfer poorly into GPT-4 hotel reviews and also degrade under the function-word attack, extending the attack's relevance to a third detector family; on RAID movie reviews (closer to their training data) they recover. This contextualises the in-domain numbers: detector quality is distribution-bound, which is the premise of this paper's transfer analysis.
